...
int main() {
    ...
    // Machine-control PPI drives persistent video/mouse interrupt latches.
    machine.io_write(0x63, 0x88);
    machine.io_write(0x60, 0x0c); // Enable capture of V pulse and mouse edges.
    machine.io_write(0x61, 7);
    assert(machine.video().scroll_latched() == 0);
    machine.video_ras_edge();
    assert(machine.video().scroll_latched() == 7);
    machine.vertical_sync_pulse();
    assert(machine.video_interrupt_pending());
    assert(machine.io_read(0x70) == 0xff); // Reading clears I3 only.
    assert(machine.video_interrupt_pending());
    machine.io_write(0xf0, 0); // Mirrored write clears I4 only.
    assert(!machine.video_interrupt_pending());
    machine.set_mouse_signals(0xfe);
    assert(machine.mouse_interrupt_pending());
    machine.io_write(0x70, 0);
    assert(machine.mouse_interrupt_pending());
    assert(machine.io_read(0x127f) == 0xfe);
    assert(!machine.mouse_interrupt_pending());

    // Activity indicators are combinational: active-low PA6 AND PC5 select.
    machine.io_write(0x60, 0x40);
    assert(!machine.drive_activity(0));
    machine.io_write(0x60, 0x00);
    machine.io_write(0x23, 0xa6);
    machine.io_write(0x22, 0x00);
    assert(machine.drive_activity(0));
    assert(!machine.drive_activity(1));
    machine.io_write(0x22, 0x20);
    assert(!machine.drive_activity(0));
    assert(machine.drive_activity(1));
