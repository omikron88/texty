...
// For WRITE transactions, controller automatically sends WREN (0x06).
    ...
    always @(posedge clk or negedge reset_n) begin
        if (!reset_n) begin
            ...
            case (state)
                ...
                ST_START_WREN: begin
                    // Single-byte WREN command: 0x06
                    spi_cs_n <= 1'b0;
                    spi_sck <= 1'b0;
                    tx_shift <= {32'h0000_0000, 8'h06};
                    bits_total <= 6'd7;
                    bit_pos <= 6'd7;
                    rx_shift <= 8'h00;
                    spi_mosi <= 1'b0; // tx_shift[7]
                    div_cnt <= CLK_DIV - 1;
                    state <= ST_TRANSFER;
                end
                ...
                ST_FINISH: begin
                    ...
                    if (op == OP_READ && bits_total == 6'd39) begin
                        ...
                        op <= OP_NONE;
                        state <= ST_IDLE;
                    end else if (op == OP_WRITE && bits_total == 6'd7) begin
                        // WREN done -> start write command.
                        state <= ST_START_RW_CMD;
                    end else begin
